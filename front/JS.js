...
==============================================================================={
	...
	var ref = new Firebase('https://project_name.firebaseIO-demo.com/folder');
	
	ref.child('child_folder') : ref  // get reference to child folder
	ref.root() : ref  // reference to root
	ref.toString() : string  // url of this folder
	
	ref.set(data [, done(err)])  // put data to this folder. previous data will
		// be lost
		...
		// Ex:
			...
			fredNameRef.set({first: 'name', last: 'surname'}, onComplete);
	
	ref.update(data)  // update without removing previous data
	ref.remove()
	ref.push([data, done(err)]) : ref  // create new child with unique name
		// Ex:
			var newMess = ref.push();
			newMess.set({'user_id': 'fred', 'text': 'Yabba Dabba Doo!'});
			// or:
	...
	// Events:
		...
		ref.on(event, callb(dataSnap))  // set handler
			// event - 'value', 'child_added', 'child_changed',
			// 'child_removed' or 'child_moved'
			// callb(dataSnap) - callback. dataSnap - data snapshot
		
		ref.once(event, callb(dataSnap))
		ref.off(event)  // remove handler
	
	================================
	// Process data snapshot:
		
		dataSnap.val() : obj  // key-value pairs
		dataSnap.hasChild("folder")
		dataSnap.child("folder")
		dataSnap.key()  // name
		dataSnap.numChildren()
}
================================================================================
RequireJS ======================================================================
==============================================================================={
